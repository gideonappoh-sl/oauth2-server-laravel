...
class FluentScope extends AbstractFluentAdapter implements ScopeInterface
{
    ...
    public function get($scope, $grantType = null, $clientId = null)
    {
        $query = $this->getConnection()
            ->table($this->getFullTableName('oauth_scopes'))
            ->select(
                $this->getFullTableName('oauth_scopes.id') . ' as id',
                $this->getFullTableName('oauth_scopes.description') . ' as description'
            )
            ->where($this->getFullTableName('oauth_scopes.id'), $scope);

        if ($this->limitClientsToScopes === true && !is_null($clientId)) {
            $query = $query
                ->join(
                    $this->getFullTableName('oauth_client_scopes'),
                    $this->getFullTableName('oauth_scopes.id'),
                    '=',
                    $this->getFullTableName('oauth_client_scopes.scope_id')
                )
                ->where($this->getFullTableName('oauth_client_scopes.client_id'), $clientId);
        }

        if ($this->limitScopesToGrants === true && !is_null($grantType)) {
            $query = $query
                ->join(
                    $this->getFullTableName('oauth_grant_scopes'),
                    $this->getFullTableName('oauth_scopes.id'),
                    '=',
                    $this->getFullTableName('oauth_grant_scopes.scope_id')
                )
                ->join(
                    $this->getFullTableName('oauth_grants'),
                    $this->getFullTableName('oauth_grants.id'),
                    '=',
                    $this->getFullTableName('oauth_grant_scopes.grant_id')
                )
